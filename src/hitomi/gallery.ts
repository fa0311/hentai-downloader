...
const getWebpUrlFromHash = (hash: string, ggJs: GGJsCode): string => {
  ...
  const directory2 = parseInt(
    dir2PartArray[2] + dir2PartArray[1],
    16,
  ).toString();

  const subdomainMatch = ggJs.case.includes(directory2)
    ? ggJs.o.match
    : ggJs.o.default;
...
export const GalleryInfoSchema = z
  .object({
    ...
    galleryurl: z.string(),
    characters: z
      .array(z.object({ character: z.string(), url: z.string() }))
      .default([]),
    ...
    type: z.string(),
    parodys: z
      .array(z.object({ parody: z.string(), url: z.string() }))
      .default([]),
...
const getGalleries = async ({
  galleryId,
  headers,
}: GetGalleriesParam): Promise<GalleryInfo> => {
  ...
  if (!response.ok) {
    throw new HentaiHttpError(
      `Failed to fetch galleries: ${galleriesUrl} - ${response.status} ${response.statusText}`,
    );
  }
...
export const downloadHitomiGalleries = async ({
  galleryId,
  additionalHeaders = {},
}: DownloadHitomiGalleriesParam) => {
  const headers = {
    ...additionalHeaders,
