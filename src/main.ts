...
const getWebpUrlFromHash = async (hash: string, domain: string, ggJs: GGJsCode): Promise<string> => {
    ...
    const subdomain = ggJs.case.includes(directory2) ? 'w' + (parseInt(ggJs.o.match) + 1) : 'w' + (parseInt(ggJs.o.default) + 1);
...
const getGalleries = async (id: string): Promise<GalleryInfo> => {
    ...
    const jsonText = galleriesJsText.replace(/^var galleryinfo = /, '').trim().replace(/;$/, '');
...
const downloadImage = async (files: { url: string; name: string }[], title: string, id: string, path: string, parallelCount: number, waitTimeMin: number, waitTimeMax: number) => {
...
const downloadManga = async (
    ...
    const files = await Promise.all(galleries.files.map(async (file) => {
    ...
    if (remainingFailed.length > 0 && config.retryCount > 0) {
        for (let attempt = 1; attempt <= config.retryCount && remainingFailed.length > 0; attempt++) {
            console.log(
                `Retrying ${remainingFailed.length} failed downloads... (attempt ${attempt}/${config.retryCount})`
            );
